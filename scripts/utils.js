...
function generateBananaPrompt(values) {
    ...
    if (realisticStyles.includes(values.style)) {
        cameraDescription = `shot on a professional DSLR like a Canon 5D, 50mm f/1.8 lens`;
    }
    
    const parts = [
        "Vertical cinematic frame, 9:16 ratio, ultra-high resolution.",
        `Core Scene: A ${values.subject} ${values.action} at ${values.location}.`,
        `Camera & Cinematography: Composition: ${values.shot}, ${cameraDescription}. Camera movement: ${values.movement}.`,
        `Lighting & Atmosphere: Lighting: ${values.lighting}. Mood & grading: ${values.mood}, styled in ${values.style}.`,
        `Details: ${practicalDetails}. Textures: photorealistic skin pores, fabric texture, natural reflections.`,
        `Post-Processing & VFX: Post-processing: ${postList}. Visual effects: ${vfxList}.`
    ];
    
    if (values.negatives.enabled && values.negatives.text) {
        parts.push(`Negative Prompts: Exclude: ${values.negatives.text}.`);
    }
...
function initializeBackground() {
    const backgroundImages = [
        ...
        'https://images.unsplash.com/photo-1576161787924-01bb08dad4a4?q=80&w=2070&auto=format&fit=crop',
